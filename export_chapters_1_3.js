...
async function main() {
  ...
  // 过滤出 封面、目录、以及 篇章1(chapter-0-)、篇章2(chapter-1-)、篇章3(chapter-2-)
  const targetSlides = slideItems.filter(item => {
    return (
      item.id === 'cover' || 
      item.id === 'toc' || 
      item.id.startsWith('chapter-0-') || 
      item.id.startsWith('chapter-1-') || 
      item.id.startsWith('chapter-2-')
    );
  });
  
  if (targetSlides.length === 0) {
    console.error('❌ 未在目录中找到属于 封面/目录/篇章1/篇章2/篇章3 的幻灯片！');
    await browser.close();
    process.exit(1);
  }

  console.log(`🎯 成功定位到目标幻灯片（共 ${targetSlides.length} 页）：`);
  ...
  for (let i = 0; i < targetSlides.length; i++) {
    const slideInfo = targetSlides[i];
    console.log(`📸 [${i + 1}/${targetSlides.length}] 正在处理: ${slideInfo.title}...`);
