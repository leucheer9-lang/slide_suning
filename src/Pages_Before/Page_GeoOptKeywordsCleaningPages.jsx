...
import FeishuKeywordsConfirmTable from '../components/FeishuKeywordsConfirmTable';
import { GEO_KEYWORDS_CONFIRM, CONFIRM_PAGE_SLICES } from '../data/geoKeywordsConfirm';

function ConfirmChrome({ sliceIndex }) {
    const [from, to] = CONFIRM_PAGE_SLICES[sliceIndex];
    const rows = GEO_KEYWORDS_CONFIRM.slice(from, to);
            ...
            <div className="flex-1 w-full px-4 lg:px-8 pb-8 mt-6 min-h-0 relative z-10 flex flex-col items-center justify-start overflow-hidden">
                <div className="w-full h-full max-w-[1700px] overflow-hidden bg-white/[0.02] backdrop-blur-md border border-white/10 rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] flex flex-col relative group transition-all duration-300 hover:border-white/20">
                    ...
                    <div className="w-full flex-1 relative min-h-0 overflow-hidden bg-[#0a0a0a]">
                        <FeishuKeywordsConfirmTable rows={rows} startIndex={from + 1} />
                    </div>
                </div>
            </div>
        </div>
...
export const PB_GeoOptKeywordsCleaningPages = CONFIRM_PAGE_SLICES.map((_, i) => {
    function Page() {
        return <ConfirmChrome sliceIndex={i} />;
    }
    Page.displayName = `Page_GeoOptKeywordsCleaning_${i + 1}`;
    return Page;
});
