          ...
          <div className="flex items-center gap-4 pl-4 mt-2">
            {activeSection.pages.map((page, j) => {
              ...
              return (
                <div
                  key={j}
                  className={`flex items-center pb-1 border-b-2 transition-colors duration-300 shrink-0 ${
                    isChildActive ? 'border-[#004CE5]' : 'border-transparent'
                  }`}
                >
                  <span
                    className={`text-[0.95rem] sm:text-base tracking-wide whitespace-nowrap ${
                      isChildActive
